...
module.exports = class BackgroundLoader extends ModeLoader {
  constructor() {
    ...
    this.defineCommand('set-acrylic', () => 'set-acrylic on | off | toggle', async argsv2 => {
      const [mode] = argsv2;
      switch (utils.uniformKeyword(mode)) {
        case 'on':
          this.jsonProfile.useAcrylic = true;
          break;
        case 'off':
          this.jsonProfile.useAcrylic = false;
          break;
        case 'toggle':
          this.jsonProfile.useAcrylic = !this.jsonProfile.useAcrylic;
          break;
        default:
          throw new Error('unsupported mode. use on, off or toggle');
      }
      return true;
    });

    this.defineCommand('set-transparent', () => 'set-transparent NUMBER (0-100)', async argsv2 => {
      const [tLvl] = argsv2;
      const pLvl = Number(tLvl) / 100;
      if (pLvl < 0 || pLvl > 1) {
        throw new Error('number must be in range [0-100]')
      }
      this.jsonProfile.backgroundImageOpacity = pLvl;
      return true;
    });
    ...
    this.defineCommand('set-webpage', () => 'set-webpage URL', async argsv2 => {
      const [url] = argsv2;
      this.jsonProfile.backgroundImage = await this.setCustomScreenshot(url);
      return true;
    });
  ...
  async setCustomImage(imgURI) {
    ...
    if (/^http(s?):\/\//i.test(imgURI)) {
      ...
      const filePath = path.join(folderCustomBgs, utils.generateRandomHex(32));
